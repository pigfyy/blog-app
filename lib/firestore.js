...
export const setPost = async (post, postId, defaultValues) => {
  ...
  if (defaultValues && slug !== defaultValues.slug) {
    const heartDocuments = await getHeartDocuments(
      userData.username,
      defaultValues.slug
    );
...
export const editHeartDocumentSlug = async (docId, slug) => {
  await updateDoc(doc(db, "users", auth.currentUser.uid, "hearts", docId), {
    slug,
  });
};
